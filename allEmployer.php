...
if(isset($_POST["s_username"])){
	...
	$sql = "SELECT * FROM employer WHERE username LIKE '%$t%'";
	$result = $conn->query($sql);
}

if(isset($_POST["s_name"])){
	$t = trim($_POST["s_name"]);
	$sql = "SELECT * FROM employer WHERE Name LIKE '%$t%'";
	$result = $conn->query($sql);
}

if(isset($_POST["s_email"])){
	$t = trim($_POST["s_email"]);
	$sql = "SELECT * FROM employer WHERE email LIKE '%$t%'";
	$result = $conn->query($sql);
}
...
<style>
	...
	.employer-table tr:hover td {
		background: #f8f9fa;
		transform: translateY(-2px);
		box-shadow: 0 5px 15px rgba(0,0,0,0.05);
	}
	.employer-table .btn-link {
		color: #000;
		font-weight: 600;
		padding: 0;
		font-size: 15px;
            ...
            <div class="card">
                <div class="panel panel-success">
                    <div class="panel-heading">
                        <h3><i class="fas fa-building"></i> All Employers</h3>
                    </div>
                                ...
                                if ($result->num_rows > 0) {
                                    while($row = $result->fetch_assoc()) {
                                        ...
                                        <input type="hidden" name="e_user" value="'.$e_username.'">
                                            <tr>
                                            <td><input type="submit" class="btn btn-link" value="'.$e_username.'"></td>
                                            <td>'.$name.'</td>
                                            <td>'.$email.'</td>
                                            <td>'.$company.'</td>
                                            </tr>
                                    ...
                                    echo '<tr><td colspan="4" class="no-results"><i class="fas fa-search"></i>No employers found</td></tr>';
                                }
                                ?>
                            </tbody>
                        </table>
